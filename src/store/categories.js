    ...
    async fetch({ commit }) {
      const categories = await get('/collections'),
        metaList = await Promise.all(
          categories.map(c => get(`/meta/collections/${c.id}`))
        )
      ...
      categories.forEach((c, i) => {
        ...
        //check & patch slides
        if (
          typeof meta.slides === 'undefined' ||
          typeof meta.slides.list === 'undefined'
        )
          meta.slides = {
            list: []
          }
        ...
        anotherPromises.push(
          ...meta.slides.list.map(slide => () =>
        ...
        if (typeof meta.settings === 'undefined') meta.settings = {}
      ...
      await promiseSerial(
        chunk(flattenDeep(anotherPromises), 4).map(chunk$1 => () =>
