...
class PushNotiMessage {
  ...
  get hasLink() {
    const {message} = this;
    return typeof message.click_action === 'string' && message.click_action.length > 0;
  }

  // "click-action" without UTM parameters
  get link() {
    if (this.hasLink)
      return this.message.click_action.split('?')[0]
    return '';
  }
  ...
  actions: {
    async fetch({commit, dispatch}) {
      ...
      if (status === 'ok') {
        ...
        dispatch('notifications/pushDanger', {
          title: 'Lỗi khi nhận danh sách tin nhắn',
          message: errorMsg
        }, {root: true})
    },
    async send({commit, dispatch, rootGetters}, {message, test = false, options = {}, stored = false}) {
      ...
      if (test)
        modifiedMessage.body = `(Sent by ${rootGetters['auth/currentUser'].name}) ` + modifiedMessage.body;

      const willSend = !stored,
        URL = willSend ? '/noti/broadcast/' : '/noti/store/';

      //enter DEBUG MODE if DEVMODE=true or explicitly set `test` param
      const {status, message: msg} = await post(URL + ((test || rootGetters['app/devMode']) ? '?debug=true' : ''), modifiedMessage);
      if (status === 'ok') {
        if (typeof msg !== 'undefined' && msg !== null)
          commit(willSend ? 'send' : 'store', msg);
        dispatch('notifications/pushSuccess', {
    ...
    async resend({commit, dispatch, rootGetters}, _id) {
      try {
        await patch('/noti/resend/?_id=' + _id + (rootGetters['app/devMode'] ? '&debug=true' : ''));
        commit('resend', _id);
        dispatch('notifications/pushSuccess', {
