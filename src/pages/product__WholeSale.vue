...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async get() {
        ...
        this.wholesale = data.map(item => {
          ...
          item.rules.forEach(rule => {
            ...
            if (!rule.description || typeof rule.description === 'string') rule.description = d(() => "");
      ...
      copyHtml({id}, index) {
        domToImage.toJpeg(this.$refs.rules[index], {
          quality: 0.7,
          ...
          this.copy = `<img data-vaithuhay-embed-type='wholesale' data-vaithuhay-embed='${(JSON.stringify(embed))}' src='${dataUrl}'>`
          // copy(this.copy)
        })
      }
    },
...
<docs>
  ...
  ### Thêm mới chính sách bán sỉ
  > Trang này được thiết kế giống với trang backend
  (https://vai-thu-hay-i-something-nice.myharavan.com/admin/app#/embed/ea0ecb2908d920fba34d52ea8b8a334a). Vì không
  đồng bộ tự động được với dữ liệu phía backend, nên cần tạo các chính sách đồng bộ với backend một cách thủ công (tên
  chính sách, số sản phẩm áp dụng và mức chiết khấu tương ứng)

  Ở cột bên trái, khung **Tạo mới chính sách bán sỉ**, nhập tên chính sách và nhấn `OK`. Nhân vào một chính sách bất
  kì ở cột bên trái để chỉnh sửa.
  ### Cập nhật chính sách bán sỉ
  #### Tab **Chi tiết**
  Bên trái để thêm mới mức chiết khấu/chỉnh sửa mức chiết khấu có sẵn. Bên phải là danh sách các mức chiết khấu

  * Ở chế độ thêm mới, nhấn `OK` sẽ thêm một mức chiết khấu vào danh sách bên phải.
  * Chọn một mức chiết khấu bên phải sẽ vào chế độ chỉnh sửa. Phần mô tả mức chiết khấu cho phép đa ngôn ngữ. Sau khi
  nhấn `OK`, tự động chuyển sang chế độ thêm mới.

  #### Tab **Áp dụng**
  Chọn thủ công từng sản phẩm được áp dụng chiết khẩu. Tìm sản phẩm ở phần bên trái, nhấn vào để áp dụng hoặc hủy áp
  dụng (danh sách được áp dụng tự động xuất hiện bên phải)
  ### Lưu chính sách bán sỉ
  Trước khi tắt ứng dụng, nhấn `Save`
</docs>
