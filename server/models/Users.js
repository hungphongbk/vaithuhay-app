...
const searchCustomerByEmailOrCreate = async (_email, {id, first_name, last_name, password}) => {
    const create = async (email) => {
        console.log('time to create');
        const {customer} = await HaravanAPI.post('/admin/customers.json')
            .json({
                customer: {
                    first_name,
                    last_name,
    ...
    const emailName = _email.split('@')[0],
        ...
        {customers} = await apiGet(`/admin/customers/search.json?query=${emailName}`, false);
...
schema.statics.createFbUser = function (accessToken, refreshToken, {_json}, cb) {
