...
const _mapHeader = {
  ...
  groupA_row: [
    'GroupA',
    index => `=iferror(VLOOKUP(T${index};'Item list (lnk)'!A:F;5;0))`
  ],
  promotion_row: [
    'Promotion',
    index => `=if(S${index}>0;(S${index}-R${index})*P${index};)`
  ],
  stock_row: [
    'Trạng thái xuất kho',
    index => `=IFERROR(VLOOKUP(A${index};'Dispatch (lnk)'!C:D;2;0);)`
  ],
  hash_row: ['Hash Validation', (_, hash) => hash]
}
const hash = (number, productId = '') =>
  stringHash(
    number.trim() +
      '@' +
      (typeof productId === 'number' ? productId : productId.trim())
  )
const randomlyFn = (fn, probability = 0.4) => {
    if (Math.random() <= probability) fn()
  },
  logger = (isEnable = process.env.NODE_ENV !== 'production') => (
    target,
    key,
    descriptor
  ) => {
...
async function getValue(_obj, key) {
  ...
  //return callback
  if (/_row$/.test(key)) return _mapHeader[key][1]
...
class Spreadsheet {
  constructor() {
    const polyfill = apiFn => {
      return (...args) =>
        apiFn.apply(null, args).then(result => {
  ...
  async _cacheSheetData(_start = null, end = '') {
    ...
    this.log(
      `Cache sheet data returned ${numbers.length}:${productIds.length} rows`
    )

    return zip(numbers, productIds)
      .map(([number, productId], rowIndex) => ({
  ...
  async _makeBody(orders) {
    ...
    updateList.sort(
      (p1, p2) => p1.sheetDataRow.rowIndex - p2.sheetDataRow.rowIndex
    )
    ...
    updateList.forEach(item => {
      if (typeof lastGroup() === 'undefined') updateGroupedList.push([item])
      else if (
        Math.abs(
          item.sheetDataRow.rowIndex -
            lastItemInLastGroup().sheetDataRow.rowIndex
        ) === 1
      )
        lastGroup().push(item)
