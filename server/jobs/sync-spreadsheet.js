...
syncQueue.process('sync', 1, async ({data: order}, done) => {
  ...
  if (order.status === 'orders/cancelled' && order.financial_status === 'pending') {
  ...
  try {
    ...
    await Promise.all(rows.map((r, index) => new Promise(async resolve => {
    ...
    const transform = (order, line_item = null) => {
      ...
      if (typeof line_item === 'number')
        rs.line_items = [line_items[line_item]];
      else if (typeof line_items === 'object')
        rs.line_items = [line_item];
      return rs;
    }
    ...
    if (founds.length === 0) {
      ...
      founds.push(...await Promise.all(order.line_items.map(line_item => {
          return new Promise(async resolve => {
            await lock.acquire();
            console.log('index = ' + rowCount);
            const r = await Row.create(sheet, rowCount++, transform(order, line_item))
            lock.release();
            resolve(r)
          })
        })
...
// ])));
      ...
      await Promise.all(founds.map((f, lineItemIndex) =>
        new Promise(resolve =>
    ...
    try {
      ...
      if (founds.length === 0) {
        ...
        for (const order of orders)
          founds.push(...(await Promise.all(order.line_items.map(line_item => new Promise(resolve =>
              Row.create(sheet, 0, transform(order, line_item, true)).then(row => resolve(row))
            )))).map(row => ({row}))
          );
        // founds.push(await Row.create(sheet, omit(order, [
        //   '_id', 'status'
        ...
        await Promise.all(founds.map(({row, order}) =>
          new Promise(resolve =>
      ...
      const updateProceeded = Promise.all(order_numbers.map(number => new Promise((resolve, reject) => {
      ...
      syncQueue.inactiveCount(function (err, remains) {
        console.log(`[CHECK-ORDER] Order ${order_numbers.join(', ')} has been updated`);
