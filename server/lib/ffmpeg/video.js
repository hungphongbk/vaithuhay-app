...
export default function(filePath, settings, infoConfiguration, infoFile) {
   ...
   */
  this.addFilterComplex = argument => {
  ...
  this.setWatermark = function(watermarkPath, settings) {
    ...
    const getVerticalMargins = (nord, sud) => {
    ...
    if (options.hasOwnProperty('video')) {
      ...
      if (options.video.hasOwnProperty('disabled')) {
        ...
        if (options.video.hasOwnProperty('size')) {
          const newDimension = _calculateNewDimension.call(this)
  ...
  var execCommand = function(callback, folder) {
    ...
    const onlyDestinationFile = folder != undefined ? false : true
    ...
    utils.ffmpegExec(finalCommands, settings, (error, stdout, stderr) => {
      // Building the result
      let result = null
      if (!error) {
        ...
        if (onlyDestinationFile) {
          ...
          for (const i in result) result[i] = [folder, result[i]].join('/')
        }
      }
  ...
  this.fnExtractSoundToMP3 = function(destionationFileName, callback) {
    ...
    const destinationDirName = path.dirname(destionationFileName),
  ...
  this.fnExtractFrameToJPG = function(/* destinationFolder, settings, callback */) {
    ...
    if (settings.file_name == null) {
      ...
      const replacements = settings.file_name.match(/(\%[a-zA-Z]{1})/g)
