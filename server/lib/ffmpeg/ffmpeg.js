...
const ffmpeg = function(/* inputFilepath, settings, callback */) {
   ...
   */
  const _ffmpegInfoConfiguration = settings => {
   ...
   * Get the video info
   */
  const _videoInfo = (fileInput, settings) => {
    // New 'promise' instance
    const deferred = when.defer()
    // Make the call to retrieve information about the ffmpeg
    utils.ffmpegExec(
   ...
   */
  const _getInformation = (fileInput, settings) => {
    const deferreds = []
    // Add promise
