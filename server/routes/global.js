...
router.get('/logs', adminMiddleware, async (req, res) => {
  ...
  const { docs: logs, total, pages } = await Log.paginate(
    {},
...
router.get('/products/:id/relatedArticles', async (req, res) => {
  ...
  const id = req.params.id,
    { metafields } = await apiGet(
      `/admin/products/${id}/metafields.json?namespace=vaithuhay`
    ),
  ...
  const meta = JSON.parse(metafield.value).relatedArticles || [],
    articles = await Promise.all(
      meta.map(async articleId => {
        const { article } = await apiGet(
          `/admin/blogs/${blogId}/articles/${articleId}.json`
        )
...
router.get(
  ...
  middlewares.allProducts,
  async (req, res) => {
    res.apicacheGroup = 'products/wholesale'
    const { i = null } = req.query
    const { data: wholesales } = await SettingsWrapper.get(
        'products',
        'wholesale'
      ),
      rs = wholesales.filter(cs => {
        return (
          cs.apply &&
          cs.apply.findIndex(id => id * 1 === req.params.id * 1) >= 0
        )
      })
    // console.log(rs);
...
router.post('/products/:id/ask', async (req, res) => {
  // try{
  const { id } = req.params,
    url = (metafieldId = null) =>
      `/admin/products/${id}/metafields${
  ...
  apiClear(url())
  if (metafield) apiClear(url(metafield.id))
  res.json({ status: 'ok' })
  // }
})
//================================================================================
router.get('/articles', async (req, res) => {
  ...
  const { count } = await apiGet(
      `/admin/blogs/${blog.id}/articles/count.json?limit=200`
    ),
      ...
      articles: flatten(
        (await Promise.all(
          range(1, pages + 1).map(page => {
            console.log(page)
            return apiGet(
              `/admin/blogs/${blog.id}/articles.json?page=${page}&limit=200`
            )
          })
        )).map(i => i.articles)
      )
...
router.get('/articles/:id/related', async (req, res) => {
  ...
  const id = req.params.id,
    { metafields } = await apiGet(
      `/admin/blogs/${
        blogs[0].id
      }/articles/${id}/metafields.json?namespace=vaithuhay&key=relatedProduct`
    )
...
router.get('/order-tracking', async (req, res) => {
  ...
  if (customerId) {
    const { orders } = await apiGet(
      `/admin/orders.json?customer_id=${customerId}&fields=${fields}`,
      false
    )
    ...
    if (kw[0] === '#') {
      ...
      const { orders } = await apiGet(
        `/admin/orders/search.json?${qs.stringify({
          fields,
          name: kw.substr(1)
        })}`,
        false
...
router.get('/meta/:resource/:id', async (req, res) => {
  log.info(
    '/meta/:resource/:id',
    `get meta from ${req.params.resource} with id = ${req.params.id}`
  )
  try {
...
router.get('/meta/:resource/:id/:resource2/:id2', async (req, res) => {
  const { resource, id, resource2, id2 } = req.params,
    { metafields } = await apiGet(
      `/admin/${resource}/${id}/${resource2}/${id2}/metafields.json`
    )
  res.json(metafields)
})
...
router.get('/meta', async (req, res) => {
  const { key, raw } = req.query,
    { metafields } = await apiGet(
      `/admin/metafields.json?namespace=vaithuhay&key=${key}`
    ),
    metafield = search(metafields, key)
  res.json(
    typeof metafield === 'undefined'
      ? []
      : flex(raw ? metafield : metafield.value)
  )
})
...
router.post('/:resource/:id/:resource2/:id2/:meta', async (req, res) => {
  try {
    ...
    if (typeof metafield === 'undefined' || metafield === null) {
      ...
      await apiPut(
        `/admin/${resource}/${id}/${resource2}/${id2}/metafields/${
          metafield.id
        }.json`,
        {
          metafield: {
