...
router.post('/manual', async (req, res) => {
  const params = {
      created_at_min: moment().subtract(15, 'days').format('YYYY-M-D HH:MM')
    },
    {count} = await apiGet('/admin/orders/count.json?' + qs.stringify(params), false);

  const orders = flatten(await Promise.all(range(Math.ceil(count / 50)).map(i =>
    apiGet('/admin/orders.json?' + qs.stringify({
      ...params,
      page: i + 1
...
if (process.env.NODE_ENV === 'development') {
  ...
  router.post('/syncOrder', async (req, res) => {
    ...
    if (number === 0) {
      params.created_at_min = moment().subtract(days, 'days').format('YYYY-M-D HH:MM');
      if (before > 0) params.created_at_max = moment().subtract(before, 'days').format('YYYY-M-D HH:MM');
    } else {
      params.limit = number;
    }

    //get exact numbers of orders & order list by _params + paginate
    const {count} = await await apiGet('/admin/orders/count.json?' + qs.stringify(params), false);
    ...
    try {
      const pushToQueue = orders => syncQueue.create('checkOrder', {orders, io: null})
        .priority('high')
        .attempts(3)
        .removeOnComplete(true)
        .save(err => {
