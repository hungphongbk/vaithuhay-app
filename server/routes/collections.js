...
const getNewProductsCollection = async () => {
    ...
    if (searched.length === 0) {
        const collection = (await getCollections()).find(c => c.handle === 'san-pham-moi');
        await (new Setting({
            namespace: 'collections',
            key: 'newProducts',
            value: collection.id
...
router.post('/new', async (req, res) => {
    ...
    const {collects: exists} = await apiGet(`/admin/collects.json?collection_id=${collection_id}`);
    products.forEach(p => {
    ...
    await Promise.all(exists
        .map(({id}) => HaravanAPI.del(`/admin/collects/${id}.json`))
    );
    await Promise.all(products.slice(0, 20).map(({id: product_id}) => HaravanAPI.post(`/admin/collects.json`)
        .json({
...
router.post('/promo', async (req, res) => {
    ...
    for (const product of saleProducts) {
        await HaravanAPI.post(`/admin/collects.json`)
            .json({
                collect: {
