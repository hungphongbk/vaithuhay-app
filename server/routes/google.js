...
import key from '../creds/gcloud/client_secret_764771183033-i9qmsuhhb4vsqh8gcd97o3f21fpm6034.apps.googleusercontent.com'
...
router.post('/lastUpdated', (req, res) => {
  updateLastTime(`server/${req.query.q}/lastUpdated`).then(() => {
    res.json({ status: 'ok' })
  })
})
...
router.post(
  ...
  async (req, res) => {
    ...
    const before10days = (d => new Date(d.setDate(d.getDate() - 10)))(
      new Date()
    )
    ...
    req.analytics.data.ga.get(
      ...
      async (err, body) => {
        if (err) {
          ...
          res.json(
            diffArray(oldTop, rs, (old, current) => old[0] === current[0])
          )
        }
      }
    )
...
router.post(
  ...
  async (req, res) => {
    ...
    const yesterday = (d => new Date(d.setDate(d.getDate() - 1)))(new Date())
    const before10days = (d => new Date(d.setMonth(d.getMonth() - 1)))(
      new Date()
    )
    const fm = d => d.toISOString().slice(0, 10)
    ...
    req.analytics.data.ga.get(
      ...
      async (err, body) => {
        if (err) {
          ...
          try {
            const { products: relateds } = await apiGet(
